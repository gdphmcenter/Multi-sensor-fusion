...
def network_encoder(encoder_input, code_size):
    X = Lambda(tf.split, arguments={'axis': 1, 'num_or_size_splits': 3})(encoder_input)
    inputA = X[0]
    inputB = X[1]
    inputC = X[2]
    ...
    z_encoded = network_encoder3(inputC, code_size)
    combinedInput = concatenate([x_encoded, y_encoded, z_encoded])
...
if __name__ == "__main__":
    ...
    model = network_cpc_Mat(image_shape=3072,terms=4,predict_terms=4,code_size=16,learning_rate=1e-3)
