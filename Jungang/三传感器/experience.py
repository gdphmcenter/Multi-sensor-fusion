...
def TSNE_Mat(code_size, signal_size=3072, cluster_num=4):
    """
...
##########################画混淆矩阵+计算正确率
def check_10_cluster(code_size, signal_size=3072):
    """
    PCA
    多次
    k_means后数据后计算轮廓系数
...
def feature_extraction(code_size=16,signal_size=3072):
    # 模型
...
def Classfier(epochs, batch_size,code_size,signal_size=3072,lr=1e-3,num_classes=4):
    '''
    进行测试
    '''
    # 加载数据
