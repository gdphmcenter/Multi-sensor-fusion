...
from CPC_model import network_encoder

MAX_STEP = 10000

log_format = 'Test_lp_loss : {:.4f}, test_lp_accuracy: {:.2f}\n'+ \
	'Test_dc_loss: {:.4f}, Test_dc_accuracy: {:.2f}\n'+ \
	'Test_target_lp_loss: {:.4f}, Test_target_lp_accuracy: {:.2f}\n'
...
class DANN_Model():
	def __init__(self, input_shape, model_type, run_name, lr, source_only=False, category=(None, None)):
		...
		#     model_type == 'B'
			...
			self.feature_extractor = Sequential([
				keras.models.Model(encoder_input, encoder_output, name='encoder')
			])

			# CPC预训练的特征提取器
			self.feature_extractor = keras.models.Model(encoder_input, encoder_output, name='encoder')
			# self.feature_extractor.load_weights(r'.\models\单域\encoderD-10.h5')
			# self.feature_extractor.load_weights(r'.\models\ABCD\ABCD-8+64+64+64.h5')
			# # #
			...
			#
			# self.feature_extractor = Sequential([
			# 	Conv2D(filters=32, kernel_size=5, strides=1, input_shape=input_shape),
			# 	BatchNormalization(),
			# 	Activation('relu'),
		...
		# 	# CPC预训练的特征提取器
		# 	self.feature_extractor = keras.models.Model(encoder_input, encoder_output, name='encoder')
		# 	self.feature_extractor.load_weights(r'.\models\encoderB-10.h5')
		#
		# 	for layer in self.feature_extractor.layers:
		# 		layer.trainable = False
		# 	self.feature_extractor.summary()
	...
	def log(self):
		message = log_format.format(
				 ...
				 self.test_lp_accuracy.result()*1,
