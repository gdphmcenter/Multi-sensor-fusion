...
class SortedGenerator_Mat(object):
    ...
    def __init__(self, batch_size, subset, terms, positive_samples=1, predict_terms=1, signal_size=2048, clusters_number=4):
...
class MatHandler_8192(object):
    ...
    def read_mat(self, dir='4分类/双传感器'):
    # def read_mat(self, dir='../4分类/02/拼接数据'):  # 读取数据  # ='../4分类/02/归一化/Z02'
        ...
        for fn in os.listdir('./'+dir):
            if fn.endswith('.mat'):
                ...
                # 剔除后面
                unwanted = now_data.shape[1] % 16384
                # print(unwanted)
                now_data = now_data[...,:-unwanted]
                # 分割数据为8192
                now_data = now_data.reshape(-1,16384)
    ...
    def load_dataset(self):
        ...
        X_test = X_test.reshape(-1, 16384, 1)
        ##################################
    ...
    def get_batch_by_labels(self, subset, labels, signal_size=1024):
        ...
        for i, label in enumerate(labels):
            ...
            idxs.append(idx_sel)
        # Process batch
        batch = X[np.array(idxs), :].reshape((len(labels), 2048, 1))
...
class MatHandler(object):
    ...
    # def read_mat(self, dir='../4分类/02/拼接数据'):
        ...
        for fn in os.listdir('./'+dir):
            if fn.endswith('.mat'):
                ...
                # now_data.sort()
                # 剔除后面
                # unwanted = now_data.shape[1] % 2048
                # print(unwanted)
                # now_data = now_data[...,:-unwanted]
                now_data = now_data[...,:300*2048]
                # print(now_data.shape)
                # 分割数据为1024
                now_data = now_data.reshape(-1,2048)
                now_data_len = now_data.shape[0]
                # 记录标签
                for layer in range(int(now_data_len)):
                    label = np.append(label, int(now_data_label))
    ...
    def get_batch_by_labels(self, subset, labels, signal_size=3072, positive_samples_n = 1, sum_term = 8):
        ...
        if positive_number > 0:
            ...
            idx_sel = np.random.choice(idx, 1)[0]
            positive_sample = X_8192[np.array(idx_sel), :].reshape((sum_term, 2048, 1))
            positive_number -= 1
        ...
        while i * sum_term <= len(labels):
            ...
            if i != 1:
                ...
                # 提取后4个
                negative_term_4 = X[np.array(temp_idxs), :].reshape((4, 2048, 1))
