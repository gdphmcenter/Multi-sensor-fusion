...
class SortedGenerator_Mat(object):
    ...
    def __init__(self, batch_size, subset, terms, positive_samples=1, predict_terms=1, signal_size=4096, clusters_number=4):
...
class MatHandler_8192(object):
    ...
    def read_mat(self, dir='../4分类/多源域/ABC/ABC-24+88+88'):  # 读取数据  # ='../4分类/02/归一化/Z02'
        # DONE
        data = np.array([],[])
        label = np.array([])
        count = 0
        ...
        for fn in os.listdir('./'+dir):
            if fn.endswith('.mat'):
                ...
                # now_data = read_data[var_DE].T  # oneD数据
                # print(now_data.shape)
                # 剔除后面
                unwanted = now_data.shape[1] % 32768
                # print(unwanted)
                # now_data = now_data[...,:-unwanted]
                now_data = now_data[...,:500*32768]
                # 分割数据为8192
                now_data = now_data.reshape(-1,32768)
    ...
    def load_dataset(self):
        ...
        X_train = X_train.reshape(-1, 32768, 1)
        X_test = X_test.reshape(-1, 32768, 1)
        ##################################
    ...
    def get_batch_by_labels(self, subset, labels, signal_size=1024):
        ...
        for i, label in enumerate(labels):
            ...
            idxs.append(idx_sel)
        # Process batch
        batch = X[np.array(idxs), :].reshape((len(labels), 4096, 1))
...
class MatHandler(object):
    ...
    def read_mat(self, dir='../4分类/多源域/ABC/ABC-24+88+88'):
    # def read_mat(self, dir='./4分类/单域/四传感器C'):
        data = np.array([],[])
        label = np.array([])
        count = 0
        # 遍历oneD文件夹中的各个mat文件
        for fn in os.listdir('./'+dir):
            if fn.endswith('.mat'):
                ...
                # now_data.sort()
                # 剔除后面
                # unwanted = now_data.shape[1] % 3072
                # print(unwanted)
                now_data = now_data[...,:300*4096]
                # print(now_data.shape)
                # 分割数据为1024
                now_data = now_data.reshape(-1,4096)
                now_data_len = now_data.shape[0]
                # 记录标签
                for layer in range(int(now_data_len)):
                    label = np.append(label, int(now_data_label))
    ...
    def get_batch_by_labels(self, subset, labels, signal_size=4096, positive_samples_n = 1, sum_term = 8):
        ...
        if positive_number > 0:
            ...
            idx_sel = np.random.choice(idx, 1)[0]
            positive_sample = X_8192[np.array(idx_sel), :].reshape((sum_term, 4096, 1))
            positive_number -= 1
        ...
        while i * sum_term <= len(labels):
            ...
            if i != 1:
                ...
                # 提取后4个
                negative_term_4 = X[np.array(temp_idxs), :].reshape((4, 4096, 1))
