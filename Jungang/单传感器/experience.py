...
def TSNE_Mat(code_size, signal_size=1024, cluster_num=4):
    """
    ...
    encoder_model.load_weights(r'.\models\encoder3#-1-1.h5')
    # 生成数据                                        
...
##########################画混淆矩阵+计算正确率
def check_10_cluster(code_size, signal_size=1024):
    """
    PCA
    多次
    k_means后数据后计算轮廓系数
...
def confusion_Matrix(confusion):
    ...
    plt.savefig('./picture/confusion/2#-1-2.png')
    plt.show()
...
def feature_extraction(code_size=16,signal_size=1024):
    ...
    encoder_model = keras.models.Model(encoder_input, encoder_output, name='encoder')
    # encoder_model.summary()
    encoder_model.load_weights(r'.\models\encoder5-64.h5')  # 加载权重文件的路径
    # 生成数据
    mathandler = MatHandler()
    data = mathandler.X_train
    label = mathandler.y_train
...
def Classfier(epochs, batch_size,code_size,signal_size=1024,lr=1e-3,num_classes=4):
    '''
    进行测试
    '''
    # 加载数据
    ...
    encoder_output = network_encoder(encoder_input, code_size)
    encoder_model = keras.models.Model(encoder_input, encoder_output, name='encoder')
    encoder_model.load_weights(r'.\models\encoder5-64.h5')
    # encoder_model.summary()
